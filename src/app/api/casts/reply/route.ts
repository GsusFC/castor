...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { accountId, content, parentHash, channelId, embeds, idempotencyKey } = body

    if (!accountId || !content || !parentHash) {
      return ApiErrors.validationFailed([
        { field: 'accountId', message: 'Required' },
        { field: 'content', message: 'Required' },
        { field: 'parentHash', message: 'Required - hash of cast to reply to' },
      ])
    }
    ...
    const idemKey = idempotencyKey
      ? `reply:openclaw:${accountId}:${parentHash}:${idempotencyKey}`
      : null
    ...
    const rateLimit = await checkRateLimit(`reply:openclaw`, 'expensive')
    if (!rateLimit.success) {
      console.warn('[Reply External] Rate limit exceeded:', ip)
      return ApiErrors.rateLimited()
    }

    console.log('[Reply External] Request:', {
      accountId,
      parentHash,
    ...
    const replyFn = async () => {
      ...
      console.log('[Reply External] Result:', result)
      ...
      if (idemKey) {
        try {
          ...
          console.warn('[Reply External] Failed to store idempotency response:', error)
        }
      }
    ...
    if (!locked.success) {
      return ApiErrors.alreadyExists('Reply already in progress')
    }
    ...
    console.error('[Reply External] Error:', error)
    return ApiErrors.operationFailed('Failed to reply to cast', {
