...
export default function ErrorBoundaryTestPage() {
  ...
  if (shouldThrow) {
    throw new Error('Error boundary test (app segment)')
  }

  return (
    <div className="mx-auto max-w-xl px-4 py-10">
      <h1 className="text-lg font-semibold text-foreground">Error boundary test (/(app))</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Pulsa el botón para lanzar un error durante el render y comprobar que se muestra
        <span className="font-medium text-foreground"> src/app/(app)/error.tsx</span>.
      </p>
