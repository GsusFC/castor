...
function getCacheTTL(feedType: string): number {
  if (feedType === 'trending') return CACHE_TTL_TRENDING
  if (feedType === 'home' || feedType === 'following') return CACHE_TTL_PERSONALIZED
...
async function handleGET(request: NextRequest) {
  try {
    ...
    const cacheControl = isPersonalized
      ? 'private, no-store'
      : `public, s-maxage=${Math.floor(cacheTTL / 1000)}, stale-while-revalidate=${Math.floor(cacheTTL / 500)}`
